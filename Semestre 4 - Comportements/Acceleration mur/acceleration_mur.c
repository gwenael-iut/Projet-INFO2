...
int parcourir(int distance)
{
    ...
    if(distance < 25) {
        fprintf(STDERR_FILENO, "La distance doit être supérieure ou égale a 25cm !");
        return 1;
    }
    ...
    distance = (buffer[2*2] | buffer[2*2+1]<<8) - distance;
    distance *= 10;
...
{
    int distance;
    printf("A quelle distance (cm) le robot doit-il s'arrêter du mur ? : ");
    scanf("%d", &distance);
