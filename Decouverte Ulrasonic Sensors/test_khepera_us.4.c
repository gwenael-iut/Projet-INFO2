...
int main(int argc, char *argv[]) {
    int rc;
    int i, position, us_activation_value;
    char Buffer[100];
    short us_values[5];
    ...
    do {
        ...
        position = -1;
        /* Valeur saisie est valide, activation des capteurs choisis */
        for (i = 0 ; (i < 16) && (position == -1) ; i++)
            if (us_activation_possibilities[i] == us_activation_value)
                position = i;
                kh4_activate_us(us_activation_value, dsPic);

        /* Valeur invalide, informe l'utilisateur et redemande le choix */
        if (position == -1)
            printf("\nValeur Invalide. Recommencez.\n\n");
